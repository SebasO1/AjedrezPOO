...
public class Rey extends Piece{
    ...
    public Rey(Board board, int col, int fil, boolean esBlanca) {
        super(board);
        this.col=col;
        this.fil=fil;
        ...
        this.sprite=sheet.getSubimage(0*Escala,esBlanca? 0:Escala,Escala,Escala).getScaledInstance(board.tamano, board.tamano, BufferedImage.SCALE_SMOOTH);
    }
    @Override
    public boolean esMovimientoValido(int col,int fil){
        return (Math.abs(this.col-col)==1 || Math.abs(this.fil-fil)==1) &&((this.col==col||this.fil==fil)||Math.abs(this.col-col)==Math.abs(this.fil-fil));
